...
import {FieldTree, form as ngForm, LogicFn, OneOrMany, PathKind, ValidationError} from '@angular/forms/signals';
...
type FormKey<T> = T extends object
  ? {
    [K in keyof T & string]:
    | K
    | (T[K] extends object ? `${K}.${FormKey<T[K]>}` : K);
  }[keyof T & string]
  : never;
...
export function required<TValue, TPathKind extends PathKind = PathKind.Root>(config?: BaseValidatorConfig<TValue, TPathKind> & {
...
export function minLength<TValue, TPathKind extends PathKind = PathKind.Root>(length: number, config?: BaseValidatorConfig<TValue, TPathKind> & {
  when?: NoInfer<LogicFn<TValue, boolean, TPathKind>>;
}) {
 // Dummy function for mocking realistic API
  return 'minLength';
